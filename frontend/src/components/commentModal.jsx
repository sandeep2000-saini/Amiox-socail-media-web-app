...
export default function CommentScreen({post}) {
  ...
  const handlePostComment = async () => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:5000/api/v1/PostImage/${post._id}/comment`,
        { text: newComment },
  ...
  return (
    <>
      ...
      <button onClick={openScreen} className="flex items-center text-gray-600 hover:text-gray-800">
      ...
      {isOpen && (
        <div
          ...
          onClick={closeScreen}
        >
          <div className="bg-white w-full max-w-md h-5/6 rounded-xl mr-5" >
            <img
          ...
          >
            ...
            <div className="p-4 border-b flex justify-between items-center">
              ...
              <span className=" ml-2" style={{fontSize:"12px"}}>{post.comments.length}</span>
              </div>
              <button
                onClick={closeScreen}
